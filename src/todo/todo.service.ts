...
export class TodoService {
 ...
 async  create(createTodoDto: any, id:number):Promise<Todo>{
    let todo:Todo = new Todo(createTodoDto)
    todo.Completed = false
    todo.Date = new Date().toLocaleDateString()
    const Mainuser =  await this.userService.findById(id)
    todo.user= Mainuser;
 ...
 async complteteUpdate(todoId:number):Promise<String>{
   let todoCompleteStatus = (await this.todoRepository.findOneById(todoId)).Completed  
   await this.todoRepository.update(todoId,{Completed:!todoCompleteStatus})
